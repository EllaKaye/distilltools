# path <- here::here("templates", "ek-talk-template.Rmd")
...
create_post_from_template <- function(path,
  ...
  if (!identical(author, "auto")) {
    ...
    supplied_author_yaml <- substr(supplied_author_yaml,
                                   start = 1,
                                   stop = nchar(supplied_author_yaml)-1)
  }
